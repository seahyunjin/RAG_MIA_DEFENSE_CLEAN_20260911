#!/usr/bin/env bash
# IA-MIA baseline pipeline (Naseh et al., "Riddle me this!" 2025).
# Configure datasets and modes in .env:
#   DATASETS="BeIR_nfcorpus BeIR_scidocs"
#   IA_MODES="default dp rerank prompt_instruction paraphrase"
#
# Each mode is run as a separate attack pass. Modes are never combined within
# one pass; setting multiple IA_MODES runs them sequentially.

set -euo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "${REPO_ROOT}"
source "${REPO_ROOT}/scripts/lib_pipeline.sh"

print_ia_usage() {
    cat <<'EOF'
Usage:
  bash scripts/run_ia_pipeline.sh --online
  bash scripts/run_ia_pipeline.sh --offline

Run --online first on a networked node, then --offline inside the GPU allocation.
All other runtime configuration is read from .env.

Environment:
  DATASETS     Space- or comma-separated dataset names to run
  IA_MODES     Space- or comma-separated modes to run sequentially
  FORCE=1      Re-run steps even when output files already exist

Modes:
  default, dp, rerank, prompt_instruction, paraphrase
EOF
}

parse_env_only_pipeline_args print_ia_usage "$@"
init_pipeline_env
set_hf_cache_args

ENV_PATH="${ENV_PATH:-.env}"
DATA_DIR="${DATA_DIR:-data}"
RESULTS_DIR="${RESULTS_DIR:-results}/IA-MIA"
NUM_QUERIES="${NUM_QUERIES:-5}"
IA_QUERY_POOL_SIZE=30
TOP_K="${TOP_K:-3}"
RAG_GENERATOR_MODEL="${RAG_GENERATOR_MODEL:-meta-llama/Llama-3.1-8B-Instruct}"
SHADOW_MODEL="${SHADOW_MODEL:-gpt-4o-mini}"
OPENAI_QUERY_MODEL="${OPENAI_QUERY_MODEL:-gpt-4.1-nano}"
RETRIEVER_MODEL="${RETRIEVER_MODEL:-sentence-transformers/all-mpnet-base-v2}"
DP_EPSILON="${DP_EPSILON:-0.1}"
DP_LEVEL="${DP_LEVEL:-output}"
RERANK_STRATEGY="${RERANK_STRATEGY:-shuffle}"
QUERY_PARAPHRASE_METHOD="${QUERY_PARAPHRASE_METHOD:-openai}"
QUERY_PARAPHRASE_MODEL="${QUERY_PARAPHRASE_MODEL:-${OPENAI_QUERY_MODEL}}"
QUERY_PARAPHRASE_TRANSFORMERS_MODEL="${QUERY_PARAPHRASE_TRANSFORMERS_MODEL:-${RAG_GENERATOR_MODEL}}"
QUERY_PARAPHRASE_BATCH_SIZE="${QUERY_PARAPHRASE_BATCH_SIZE:-32}"
PIPELINE_PHASE="${PIPELINE_PHASE:-all}"
apply_pipeline_phase_arg

DATASET_CONFIG="${DATASETS:-BeIR_nfcorpus BeIR_scidocs BeIR_trec-covid}"
split_pipeline_env_list DATASETS "${DATASET_CONFIG}"
if [[ ${#DATASETS[@]} -eq 0 ]]; then
    echo "Error: no datasets selected. Set DATASETS in ${ENV_PATH}." >&2
    exit 2
fi

RAW_IA_MODES=()
split_pipeline_env_list RAW_IA_MODES "${IA_MODES:-default}"
if [[ ${#RAW_IA_MODES[@]} -eq 0 ]]; then
    echo "Error: no modes selected. Set IA_MODES in ${ENV_PATH}." >&2
    exit 2
fi
IA_RUN_MODES=()
for raw_mode in "${RAW_IA_MODES[@]}"; do
    IA_RUN_MODES+=("$(normalize_attack_mode "${raw_mode}")")
done
dedupe_default_first_modes IA_RUN_MODES

RETRIEVER_TAG="${RETRIEVER_MODEL//\//--}"
GENERATOR_TAG="${RAG_GENERATOR_MODEL//\//--}"

echo "IA-MIA query pool: queries_${IA_QUERY_POOL_SIZE}.jsonl (${IA_QUERY_POOL_SIZE} questions/doc)."
echo "IA-MIA evaluation uses NUM_QUERIES=${NUM_QUERIES}."
echo "IA-MIA modes: ${IA_RUN_MODES[*]}"
echo "Datasets: ${DATASETS[*]}"
print_pipeline_phase_summary
echo ""

for ds in "${DATASETS[@]}"; do
    echo "=================================================="
    echo "Dataset: ${ds}"
    echo "Shared IA-MIA setup"
    echo "=================================================="

    member="${DATA_DIR}/${ds}/corpus_member.jsonl"
    nonmember="${DATA_DIR}/${ds}/corpus_nonmember.jsonl"
    queries_file="$(resolve_ia_queries_file "${ds}" "${RESULTS_DIR}")"
    queries_dir="$(dirname "${queries_file}")"
    gt_file="${RESULTS_DIR}/${ds}/gt/gt.jsonl"

    if ! pipeline_run_online_enabled; then
        echo "[SKIP] [1/5] IA query pool (${queries_file##*/}) (online phase only)"
    elif skip_if_exists "${queries_file}" "[1/5] IA query pool (${queries_file##*/})"; then
        :
    else
        echo "[1/5] Generating IA query pool (${IA_QUERY_POOL_SIZE} questions/doc) ..."
        python IA-MIA/generate_queries.py \
            --corpus_member "${member}" \
            --corpus_nonmember "${nonmember}" \
            --output_dir "${queries_dir}" \
            --num_questions "${IA_QUERY_POOL_SIZE}" \
            --generation_method openai \
            --openai_model "${OPENAI_QUERY_MODEL}"
    fi

    if ! pipeline_run_online_enabled; then
        echo "[SKIP] [3/5] Generating shadow ground-truth answers (online phase only)"
    elif skip_if_exists "${gt_file}" "[3/5] Generating shadow ground-truth answers"; then
        :
    else
        echo "[3/5] Generating shadow ground-truth answers with ${SHADOW_MODEL} ..."
        python IA-MIA/generate_ground_truth.py \
            --queries_file "${queries_file}" \
            --corpus_member "${member}" \
            --corpus_nonmember "${nonmember}" \
            --output_file "${gt_file}" \
            --generation_method openai \
            --openai_model "${SHADOW_MODEL}"
    fi
done

run_ia_mode() {
    local run_mode="$1"
    validate_attack_mode_config "${run_mode}"

    echo "=================================================="
    echo "IA-MIA mode: ${run_mode}"
    echo "=================================================="

    local defense_args=()
    build_defense_args defense_args "${run_mode}"

    for ds in "${DATASETS[@]}"; do
        echo "=================================================="
        echo "Dataset: ${ds}"
        echo "Mode: ${run_mode}"
        echo "=================================================="

        local member="${DATA_DIR}/${ds}/corpus_member.jsonl"
        local nonmember="${DATA_DIR}/${ds}/corpus_nonmember.jsonl"
        local index="${DATA_DIR}/${ds}/faiss_indices/${RETRIEVER_TAG}.faiss"
        local summary="${DATA_DIR}/${ds}/summary.jsonl"
        local base_queries_file
        base_queries_file="$(resolve_ia_queries_file "${ds}" "${RESULTS_DIR}")"
        local queries_file="${base_queries_file}"
        local retrieval_dir="${RESULTS_DIR}/${ds}/retrieval"
        local answers_dir
        answers_dir="$(mode_dirname answers "${run_mode}")"
        local eval_base_dir
        eval_base_dir="$(mode_dirname evaluation "${run_mode}")"

        if [[ "${run_mode}" == "paraphrase" ]]; then
            queries_file="${base_queries_file/\/queries\//\/queries_paraphrased\/}"
            retrieval_dir="${RESULTS_DIR}/${ds}/retrieval_paraphrased"

            if [[ "${QUERY_PARAPHRASE_METHOD}" == "openai" ]] && ! pipeline_run_online_enabled; then
                echo "[SKIP] [1b/5] Paraphrasing IA queries (online phase only)"
            elif [[ "${QUERY_PARAPHRASE_METHOD}" != "openai" ]] && ! pipeline_run_offline_enabled; then
                echo "[SKIP] [1b/5] Paraphrasing IA queries (offline phase only)"
            elif skip_if_exists "${queries_file}" "[1b/5] Paraphrasing IA queries"; then
                :
            else
                echo "[1b/5] Paraphrasing IA queries ..."
                local paraphrase_gpu_args=()
                if [[ "${QUERY_PARAPHRASE_METHOD}" == "transformers" ]]; then
                    paraphrase_gpu_args=(--use_gpu)
                fi
                python defense/query_paraphrase.py \
                    --ia_mia_queries "${base_queries_file}" \
                    --dcmi_queries "" \
                    --generation_method "${QUERY_PARAPHRASE_METHOD}" \
                    --openai_model "${QUERY_PARAPHRASE_MODEL}" \
                    --transformers_model "${QUERY_PARAPHRASE_TRANSFORMERS_MODEL}" \
                    --generation_batch_size "${QUERY_PARAPHRASE_BATCH_SIZE}" \
                    "${HF_CACHE_ARGS[@]}" \
                    "${paraphrase_gpu_args[@]}"
            fi
        fi

        if ! pipeline_run_offline_enabled; then
            echo "[STOP] Online phase completed for ${ds} (${run_mode}); skipping offline GPU/local stages."
            continue
        fi
        local retrieval_file="${retrieval_dir}/topk${TOP_K}_results.json"
        local retrieval_force_mode="mode"
        if [[ "${run_mode}" != "default" && "${run_mode}" != "paraphrase" ]]; then
            retrieval_force_mode="shared_prereq"
        fi
        local gt_file="${RESULTS_DIR}/${ds}/gt/gt.jsonl"
        local answer_dir="${RESULTS_DIR}/${ds}/${answers_dir}/topk${TOP_K}"
        local answer_file="${answer_dir}/answers_${GENERATOR_TAG}.jsonl"
        local eval_dir="${RESULTS_DIR}/${ds}/${eval_base_dir}/topk${TOP_K}/${GENERATOR_TAG}"
        local eval_marker="${eval_dir}/evaluation_q${NUM_QUERIES}.json"

        if [[ "${retrieval_force_mode}" == "shared_prereq" && -s "${retrieval_file}" ]]; then
            echo "[SKIP] [2/5] Ensuring top-${TOP_K} contexts"
            echo "       reusing shared retrieval: ${retrieval_file}"
        elif [[ "${retrieval_force_mode}" == "mode" ]] && skip_if_exists "${retrieval_file}" "[2/5] Retrieving top-${TOP_K} contexts"; then
            :
        else
            echo "[2/5] Retrieving top-${TOP_K} contexts ..."
            python IA-MIA/retrieve.py \
                --queries_file "${queries_file}" \
                --index_path "${index}" \
                --corpus_member_file "${member}" \
                --corpus_nonmember_file "${nonmember}" \
                --summary_file "${summary}" \
                --output_dir "${retrieval_dir}" \
                --retrieval_model "${RETRIEVER_MODEL}" \
                "${HF_CACHE_ARGS[@]}" \
                --top_k "${TOP_K}" \
                --use_gpu
        fi

        if skip_if_exists "${answer_file}" "[4/5] Generating RAG answers"; then
            :
        else
            echo "[4/5] Generating RAG answers with ${RAG_GENERATOR_MODEL} ..."
            mkdir -p "${answer_dir}"
            python IA-MIA/generate_rag_answers.py \
                --queries_file "${queries_file}" \
                --retrieval_file "${retrieval_file}" \
                --output_file "${answer_file}" \
                --corpus_member_file "${member}" \
                --corpus_nonmember_file "${nonmember}" \
                --generation_method transformers \
                --transformers_model "${RAG_GENERATOR_MODEL}" \
                "${HF_CACHE_ARGS[@]}" \
                --max_context_docs "${TOP_K}" \
                "${defense_args[@]}" \
                --use_gpu
        fi

        if skip_if_exists "${eval_marker}" "[5/5] Evaluating"; then
            :
        else
            if [[ "${FORCE}" == "1" ]]; then
                rm -f "${eval_dir}/mia_scores_q${NUM_QUERIES}.csv"
            fi
            echo "[5/5] Evaluating ..."
            mkdir -p "${eval_dir}"
            python IA-MIA/evaluate.py \
                --rag_answers "${answer_file}" \
                --gt_answers "${gt_file}" \
                --corpus_member "${member}" \
                --corpus_nonmember "${nonmember}" \
                --num_queries "${NUM_QUERIES}" \
                --output_dir "${eval_dir}"
        fi
    done
}

for run_mode in "${IA_RUN_MODES[@]}"; do
    run_ia_mode "${run_mode}"
done

echo "IA-MIA pipeline finished for phase ${PIPELINE_PHASE}. Metrics under ${RESULTS_DIR}/<dataset>/evaluation*/..."
