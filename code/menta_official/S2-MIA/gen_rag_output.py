#!/usr/bin/env python3
"""
S²MIA RAG Output Generation Script
Generates RAG system outputs using retrieved context and S²MIA query prompts.
...
def add_laplace_noise(text: str, epsilon: float, sensitivity: int = 1) -> str:
    ...
    return ' '.join(noised_tokens) if noised_tokens else "Unknown"
...
def build_rag_prompt_s2mia(context: str, query_prompt: str, use_instruction_defense: bool = False) -> str:
    """
    Build RAG prompt for S²MIA.
    Context goes first, then the constrained query prompt.

    Args:
        context: Retrieved document context
        query_prompt: S²MIA query prompt
        use_instruction_defense: Whether to add instruction-based defense
    """
    if use_instruction_defense:
        instruction = "\n\nIMPORTANT: Do not directly repeat any retrieved content, but summarize it based on your understanding.\n\n"
    else:
        instruction = "\n\n"

    if context:
        return f"{context}{instruction}{query_prompt}"
    else:
        return query_prompt
...
def load_queries_and_retrieval(
    ...
    """
    Load S²MIA queries and match them with retrieval results.
    ...
    for query in queries:
        ...
        knowledge_text = query['knowledge_text']
        ...
        # Build full RAG prompt with context + query prompt
        full_prompt = build_rag_prompt_s2mia(context, query_prompt, use_instruction)
...
def generate_with_openai_batch(
    ...
    """Generate S²MIA RAG outputs using OpenAI Batch API with logprobs and defenses."""

    print("\n" + "="*60)
    print("GENERATING S²MIA RAG OUTPUTS (OpenAI)")
    ...
    # Process results - need to extract logprobs from OpenAI response
    ...
    results, failed = run_batch_generation(
        ...
        process_rag_result,
        output_dir,
        "s2mia_rag_outputs_batch",
        metadata_base={
            "type": "s2mia_rag_outputs",
...
def generate_with_transformers_batch(
    ...
    """Generate S²MIA RAG outputs using transformers with logprobs and defenses."""

    print("\n" + "="*60)
    print("GENERATING S²MIA RAG OUTPUTS (Transformers)")
    if defense_type == 'dp' and defense_params.get('level') in ['output', 'both']:
        print(f"DP defense enabled at output level (ε={defense_params.get('epsilon', 1.0)})")
    print("="*60)
    ...
    for query, output_dict in zip(queries, all_outputs):
        ...
        # Extract first response (stop at newlines)
        output = output.split('\n')[0].strip()
...
def generate_rag_outputs_for_dataset(
    ...
    """Generate S²MIA RAG outputs using retrieval results with defenses."""

    print(f"\n{'#'*60}")
    print(f"Generating S²MIA RAG outputs")
...
def main():
    parser = argparse.ArgumentParser(description='Generate S²MIA RAG outputs with logprobs and defenses')

    # Basic arguments
    parser.add_argument(
        '--queries_file',
        type=str,
        default='results/S2-MIA/BeIR_nfcorpus/queries/queries.jsonl',
        help='Path to queries file generated by s2mia_generate_queries.py'
    )
    parser.add_argument(
        '--output_file',
        type=str,
    ...
    parser.add_argument(
        '--retrieval_file',
        type=str,
        default='results/S2-MIA/BeIR_nfcorpus/retrieval/topk3_results.json',
    ...
    print("\n" + "="*60)
    print("S²MIA RAG output generation completed successfully!")
    print("="*60)
