#!/usr/bin/env python3
"""Exp203 selective QLL-located source suppression with deterministic backfill.

This is a one-factor causal follow-up to Exp202. The frozen cohort, QLL
locator, dominance threshold, total context budget, and output budget are
unchanged. Only the intervention action changes: the selected source is
fully excluded instead of exposing its first 64 tokens.
"""
...
ROOT = PROJECT / "exp203_selective_qll_source_hide_20260830"
EXP202 = PROJECT / "exp202_selective_qll_stable_cap_20260830"
...
PACKING = ROOT / "private/EXP203_PACKING.private.pkl.gz"
RESPONSES = ROOT / "private/EXP203_RESPONSES.private.csv.gz"
GEN_DB = ROOT / "private/EXP203_QWEN_RESPONSES.sqlite3"
CANDIDATE = "SELECTIVE_QLL_SOURCE_HIDE_BACKFILL_FIXED64"
...
common = load(EXP199 / "code/run_exp199.py", "exp203_common")
...
def checkpoint(stage, **values):
    payload = {"experiment": "Exp203", "candidate": CANDIDATE, "stage": stage,
    ...
    lines = ["# Exp203 Status", "", f"- Stage: **{stage}**"] + [f"- {k}: `{v}`" for k, v in values.items()]
...
def preflight():
    ...
    required = {"precommit": PRECOMMIT, "qll": QLL, "attacks": ATTACKS, "normal": NORMAL,
        ...
        "exp202_result": EXP202 / "FINAL_RESULT.json",
        "exp202_generation_cache": EXP202 / "private/EXP202_QWEN_RESPONSES.sqlite3",
        "mirabel_locator_audit": PROJECT / "exp181_mirabel_locator_rank_audit_20260828/FINAL_RESULT.json"}
    ...
    common.atomic_json(ROOT / "audits/MECHANISM_AUDIT.json", {
        ...
        "normal_calibration_scope": "frozen Exp202 TopiOCQA development threshold",
        "selected_source_action": "FULL_EXCLUSION", "deterministic_backfill": True,
        "session_ledger": False,
        "causal_change_from_exp202": "selected source Prefix64 -> full source exclusion",
        "training": False})
...
def load_policy_rows():
    ...
    frozen_threshold = float(json.loads((EXP202 / "FINAL_RESULT.json").read_text())["threshold"])
    configured_threshold = float(json.loads(PRECOMMIT.read_text())["policy"]["frozen_dominance_threshold"])
    if not np.isclose(threshold, frozen_threshold, atol=1e-15, rtol=0.0):
        raise RuntimeError(f"Exp202 threshold drift: recomputed={threshold}, frozen={frozen_threshold}")
    if not np.isclose(threshold, configured_threshold, atol=1e-15, rtol=0.0):
        raise RuntimeError(f"precommitted threshold mismatch: recomputed={threshold}, config={configured_threshold}")
...
def phase_a(attacks, normal, qll, threshold, config):
    ...
    common.atomic_csv(rank, ROOT / "tables/TABLE_203_01_PHASE_A_RANK_GATE.csv")
    ...
    common.atomic_csv(pd.DataFrame(rows), ROOT / "tables/TABLE_203_02_PHASE_A_FAMILY.csv")
    ...
    common.atomic_csv(gates, ROOT / "tables/TABLE_203_03_PHASE_A_GATES.csv")
...
def render(tokenizer, models, query, source_texts, caps):
    ...
    for text, cap in zip(source_texts, caps):
        ...
        used.append(len(ids))
        if ids:
            contexts.append(tokenizer.decode(ids, skip_special_tokens=True).strip())
...
def build_packing(attacks, normal, threshold):
    ...
    def add(row, condition, trigger):
        ...
        used_sources = [source for source, cap in zip(sources, caps) if cap > 0]
        rows.append({"case_id": str(row.case_id), "row_id": str(row.row_id), "panel": str(row.panel),
            ...
            "selected_source_rank": selected + 1 if selected is not None else 0,
            "source_ids_retrieved": sources, "source_ids_used": used_sources,
    ...
    common.atomic_csv(summary, ROOT / "tables/TABLE_203_04_PACKING.csv")
    checkpoint("PACKING_COMPLETE", rows=len(output), unique_tasks=int(output.generation_key.nunique()))
    return output


def task_signature(task):
    """Return the exact frozen-generation signature used for safe reuse."""
    return json.dumps({
        "generator": task.get("generator"),
        "generator_revision": task.get("generator_revision"),
        "system_prompt_hash": task.get("system_prompt_hash"),
        "prompt_hash": task.get("prompt_hash"),
        "generation_config": task.get("generation_config"),
        "max_new_tokens": task.get("max_new_tokens"),
    }, sort_keys=True, separators=(",", ":"))


def seed_exact_task_cache(tasks, models):
    """Reuse Exp202 only when the complete generation signature is identical."""
    source = EXP202 / "private/EXP202_QWEN_RESPONSES.sqlite3"
    connection = sqlite3.connect(f"file:{source}?mode=ro", uri=True)
    frozen = {}
    for task_json, response in connection.execute("SELECT task_json,response_text FROM responses"):
        task = json.loads(task_json)
        frozen.setdefault(task_signature(task), str(response))
    connection.close()

    store = models.ResponseStore(GEN_DB)
    already_present = 0
    reused = 0
    for task in tasks:
        if store.get(str(task["task_key"])) is not None:
            already_present += 1
            continue
        response = frozen.get(task_signature(task))
        if response is not None:
            store.put(task, response, f"EXACT_TASK_SIGNATURE:{source}")
            reused += 1
    store.close()
    audit = {
        "source": str(source), "source_sha256": common.sha256_file(source),
        "access": "READ_ONLY", "source_rows": len(frozen), "tasks": len(tasks),
        "already_present": already_present, "exact_task_signature_reused": reused,
        "matched_fields": ["generator", "generator_revision", "system_prompt_hash",
            "prompt_hash", "generation_config", "max_new_tokens"]}
    common.atomic_json(ROOT / "audits/EXACT_TASK_CACHE_AUDIT.json", audit)
    return audit
...
def generate(packing):
    ...
    cache_audit = seed_exact_task_cache(tasks, models)
...
def evaluate(responses, config):
    ...
    module = load(EXP188 / "code/run_exp188.py", "exp203_external"); module.ROOT = ROOT
    ...
    privacy = pd.DataFrame(privacy_rows); common.atomic_csv(privacy, ROOT / "tables/TABLE_203_05_BUDGETLEAK.csv")
    ...
    core = load(EXP188 / "code/exp188_core.py", "exp203_core")
    ...
    utility = normal.groupby("condition",as_index=False).agg(rows=("row_id","size"),token_f1=("token_f1","mean"),
        ...
        request_block_rate=("request_blocked","mean"))
    common.atomic_csv(utility, ROOT / "tables/TABLE_203_06_NORMAL_UTILITY.csv")
    ...
    common.atomic_csv(gates, ROOT / "tables/TABLE_203_07_GATES.csv")
    passed=bool(gates.passed.all()); verdict="SELECTIVE_QLL_HIDE_BUDGETLEAK_PASS" if passed else "SELECTIVE_QLL_HIDE_BUDGETLEAK_FAIL"
    generation_audit_path = ROOT / "audits/GENERATION_PROGRESS_GENERATION_AUDIT.json"
    generation_audit = json.loads(generation_audit_path.read_text()) if generation_audit_path.exists() else {}
    cache_audit_path = ROOT / "audits/EXACT_TASK_CACHE_AUDIT.json"
    cache_audit = json.loads(cache_audit_path.read_text()) if cache_audit_path.exists() else {}
    result={"experiment":"Exp203","candidate":CANDIDATE,"verdict":verdict,"passed":passed,
      ...
      "generation_tasks":int(responses.generation_key.nunique()),
      "exact_cache_reused":int(cache_audit.get("exact_task_signature_reused", 0)),
      "new_local_forwards":int(generation_audit.get("new_local_forwards", responses.generation_key.nunique())),
      "selected_source_action":"FULL_EXCLUSION", "completed_utc":now()}
    common.atomic_json(ROOT/"FINAL_RESULT.json",result)
    report=f"""# Exp203 — Selective QLL Source Suppression

- 최종 판정: **{verdict}**
- Exp202와 동결된 QLL/threshold/cohort/context/output 조건은 동일하다.
- 유일한 인과 변경: 선택 source Prefix64 -> source 전체 제외 후 deterministic backfill.
- 정상 개발 intervention target: 5%; 요청 차단: 0%; 출력 budget: 64
- QLL은 top-4를 한 batch로 teacher-forced scoring하고 답변은 한 번만 생성한다.
- 이번 단계에는 cross-query session ledger가 없으며 BudgetLeak 단일-query 반복 budget만 평가한다.
...
"""
    common.atomic_text(ROOT/"reports/REPORT_203_FINAL_KO.md",report); checkpoint("COMPLETE",verdict=verdict)
...
def main():
    ...
    if not phase_a(attacks,normal,qll,threshold,config):
        common.atomic_json(ROOT/"FINAL_RESULT.json",{"experiment":"Exp203","verdict":"PHASE_A_FAIL","completed_utc":now()})
