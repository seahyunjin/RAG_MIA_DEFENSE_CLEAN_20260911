...
def score_queries(query_rows: list[dict]) -> tuple[list[dict], dict]:
    ...
    member_bad = [row["target_id"] for row in query_rows if row["membership"] == "member" and row["target_id"] not in doc_id_set]
    nonmember_bad = [row["target_id"] for row in query_rows if row["membership"] == "nonmember" and row["target_id"] in doc_id_set]
    if member_bad or nonmember_bad:
        raise RuntimeError(f"IA canonical membership mapping invalid: member_missing={len(member_bad)} nonmember_inside={len(nonmember_bad)}")
