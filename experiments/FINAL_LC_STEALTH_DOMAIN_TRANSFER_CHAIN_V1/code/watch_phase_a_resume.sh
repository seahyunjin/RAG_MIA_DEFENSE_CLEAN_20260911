...
LOG="$EXP/logs/CHAIN_RESUME.log"
LOCK="$EXP/runtime/chain-resume.lock"
...
note "Frozen Phase-A resume chain started pid=$$ cap=USD12.00"
run_stage IA_API_FULL_RESUME "$PY" "$CODE/run_phase_c_api_resume.py" || exit 21
if ! grep -q '"verdict": "IA_STD_Q15_API1_READY"' "$IA/FINAL_RESULT.json"; then
  note "IA full protocol did not reach READY; strict chain stopped"
  exit 0
fi
if [[ ! -f "$IA/post_ready/configs/IA_API1_DETECTION_PRECOMMIT.json" ]]; then
  run_stage IA_DETECTION_PREPARE "$PY" "$CODE/prepare_phase_c_detection.py" || exit 22
fi
if [[ ! -f "$IA/post_ready/IA_API1_DETECTION_RESULT.json" ]]; then
  run_stage IA_DETECTION "$PY" "$CODE/run_phase_c_detection.py" || exit 23
fi
if ! grep -q '"verdict": "IA_STEALTH_DETECTION_PASS"' "$IA/post_ready/IA_API1_DETECTION_RESULT.json"; then
  note "IA_STEALTH_DETECTION_NOT_SUPPORTED; E2E not started"
  exit 0
fi
if [[ ! -f "$IA/post_ready/configs/IA_API1_E2E_PRECOMMIT.json" ]]; then
  run_stage IA_E2E_PREPARE "$PY" "$CODE/prepare_phase_c_e2e.py" || exit 24
fi
if [[ ! -f "$IA/post_ready/runtime/IA_ST1_GENERATION_MANIFEST.json" ]]; then
  run_stage IA_E2E_GENERATION "$PY" "$CODE/run_phase_c_e2e_generation.py" || exit 25
fi
if [[ ! -f "$IA/post_ready/IA_API1_E2E_RESULT.json" ]]; then
  run_stage IA_E2E_SCORING "$PY" "$CODE/run_phase_c_e2e_scoring.py" || exit 26
fi
if grep -q '"verdict": "IA_STEALTH_CONFIRMATION_PASS"' "$IA/post_ready/IA_API1_E2E_RESULT.json"; then
  note "IA_STEALTH_CONFIRMATION_PASS"
else
  note "IA_STEALTH_E2E_FAILED"
fi
