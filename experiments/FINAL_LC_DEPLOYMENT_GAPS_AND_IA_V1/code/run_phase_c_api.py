...
def main() -> None:
    ...
    preflight_result = {
        ...
        **summary, "checks": checks,
        "verdict": "IA_STD_Q15_API1_PREFLIGHT_PASS" if passed else "IA_STD_Q15_API1_UNAVAILABLE",
    ...
    if not passed:
        ...
        checkpoint("IA_STD_Q15_API1_UNAVAILABLE", valid=f"{summary['valid']}/100", gap=summary["validity_gap"])
        con.close(); print(json.dumps(preflight_result, indent=2)); return

    checkpoint("IA_STD_Q15_API1_PREFLIGHT_PASS", valid=f"{summary['valid']}/100", next="FULL_2000")
    ...
    if all(full_checks.values()):
        ...
        result = {
            "campaign": EXP.name, "attack": "IA-Std-Q15-API1", "completed_utc": now(),
            "verdict": "IA_STD_Q15_API1_UNAVAILABLE", **full, "checks": full_checks,
        ...
        atomic_json(IA / "FINAL_RESULT.json", result)
        checkpoint("IA_STD_Q15_API1_UNAVAILABLE", valid=f"{full['valid']}/2000", gap=full["validity_gap"])
