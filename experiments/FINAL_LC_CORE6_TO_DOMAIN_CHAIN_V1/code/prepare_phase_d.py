...
from common import (BGE, EXP, LC, QWEN, ROOT, SEED, checkpoint, freeze_json,
...
def main() -> None:
    phase_c=json.loads((EXP/"PHASE_C_RESULT.json").read_text(encoding="utf-8"))
    if phase_c["verdict"]!="GOLD_QA_UTILITY_PASS":
        raise RuntimeError("Phase D prohibited because Phase C did not pass")
    ...
    for membership,ids in (("member",member_ids),("nonmember",nonmember_ids)):
        ...
        for index,report_id in enumerate(members):
            row=by_id[report_id]
            targets.append({"target_id":row["document_id"],"source_id":report_id,"membership":membership,
    ...
    code=[EXP/"code"/name for name in ("common.py","prepare_phase_d.py","run_phase_d_queries.py","run_phase_d.py","run_phase_d_e2e.py")]
    ...
    pre={"campaign":EXP.name,"phase":"D_NEW_DOMAIN_GENERALIZATION","created_utc":now(),"status":"PHASE_D_PRECOMMITTED",
        ...
        "strict_thresholds":{f"{row['method']}@{row['nominal_budget']}":row for row in phase_a["benign_alarm_rows"]},
        "primary_budget":.025,"screen":{"member":100,"nonmember":100},"scale":{"member":1000,"nonmember":1000},
        "attacks":["MEntA","RAG-MIA","IA-Std-Q15"],
        ...
        "generalization_gate":{"actual_benign_fpr":"<=0.05","privacy":"all three improve vs No Defense","matched_mirabel":"noninferior within 0.03","gold_qa_drop":"<=0.02 absolute","training_updates":0},
